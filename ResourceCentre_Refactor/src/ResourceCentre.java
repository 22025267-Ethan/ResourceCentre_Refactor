...
public class ResourceCentre {
	...
	public static void main(String[] args) {
		...
		while (option != OPTION_QUIT) //CONSTANT {

			ResourceCentre.menu();
			option = Helper.readInt("Enter an option > ");

			if (option == OPTION_VIEW) { //CONSTANT
				// View all items
				ResourceCentre.viewAllCamcorder(camcorderList);
				ResourceCentre.viewAllChromebook(chromebookList);

			} else if (option == OPTION_ADD) { // CONSTANT
				// Add a new item
			...
			} else if (option == OPTION_LOAN) { //CONSTANT
				// Loan item
				...
				if (itemType == ITEM_TYPE_CAMCODER) { //CONSTANT
					// Loan camcorder
					ResourceCentre.loanCamcorder(camcorderList);
				} else if (itemType == ITEM_TYPE_CHROMEBOOK)  { // CONSTANT
					// Loan Chromebook
			...
			} else if (option == OPTION_RETURN) { // CONSTANT
				// Return item
			...
			} else if (option == OPTION_QUIT)  { //constant
	...
	public static String retrieveAllCamcorder(ArrayList<Camcorder> camcorderList) {
		...
		for (int i = 0; i < camcorderList.size(); i++) {
			...
			String assetTag = camcorderList.get(i).getAssetTag(); // extract local variable 
